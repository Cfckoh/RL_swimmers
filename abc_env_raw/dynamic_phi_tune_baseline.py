...
BASELINE_SCALE = 1.
SEPERATION_DISCOUNT = 0.1#1.5#0.25
...
class DynamicPhiAgent:
    def __init__(self, state_size=3, hidden_size=64, num_layers=4, actor_lr=0.000025, discount=1., action_scale=1.):
    ...
    def train(self, state_list, selected_phi_list, reward_list, next_state_list, baseline_aproximator, time_remaining, time_step, decay_term, verbose=False):
        ...
        approx_value = -quadratic_cost(torch.linalg.norm(state_t, axis=1),1.1609802 , 0.04960699).view(-1,1)#baseline_aproximator.evaluate_prescribed(torch.linalg.norm(state_t, axis=1)).view(-1,1)
        sampled_q_val = reward_t + -quadratic_cost(torch.linalg.norm(state_t, axis=1),1.1609802 , 0.04960699).view(-1,1)#baseline_aproximator.evaluate_prescribed(torch.linalg.norm(next_state_t, axis=1)).view(-1,1)
...
if __name__=="__main__":
    ...
    #        time.sleep(5)
        ...
        np.savetxt(f"csv/loss_phi={baseline_phi/10}.csv", stats_actor_loss, delimiter=",")
        np.savetxt(f"csv/rewards_phi={baseline_phi/10}.csv", rewards_per_episode, delimiter=",")
        ...
        agent.save_policy(f"saved_models/phi_only_{baseline_phi/10}.pt")
